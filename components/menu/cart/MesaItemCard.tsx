...
interface MesaItemCardProps {
  item: Item
  qty: number
  onAdd: () => void
}

export function MesaItemCard({ item, qty, onAdd }: MesaItemCardProps) {
  const isSoldOut = !item.is_active

  return (
    <div className="relative w-72 flex-shrink-0">
      {qty > 0 && (
        <div className="absolute top-1 right-1 z-20 w-7 h-7 rounded-full bg-white flex items-center justify-center shadow-md">
          <span className="font-outfit font-bold text-[13px]" style={{ color: "var(--accent)" }}>{qty}</span>
        </div>
      )}
      ...
      <div className="absolute -top-12 left-1/2 -translate-x-1/2 z-10 w-40 h-40 rounded-full overflow-hidden ring-[3px] ring-white shadow-[0_8px_28px_rgba(0,0,0,0.22)] bg-white">
        {item.image_url ? (
          ...
          <div className="w-full h-full flex items-center justify-center" style={{ background: "rgba(255,255,255,0.18)" }}>
      ...
      <div
        className={`rounded-[28px] min-h-[380px] flex flex-col overflow-hidden relative ${isSoldOut ? "shadow-[0_6px_28px_rgba(0,0,0,0.1)]" : "shadow-[0_6px_28px_rgba(0,0,0,0.13)]"}`}
        style={{
      ...
      >
        <div
          ...
          style={{ backgroundImage: "url('/fundo-sem-fundo.png')", opacity: 0.20, filter: "brightness(0) invert(1)" }}
        />
        ...
        <div className="relative pt-32 px-5 pb-6 flex flex-col flex-1">
          ...
          <div className="mt-4 flex justify-center">
            <span className={`inline-block font-outfit font-semibold text-[13px] px-4 py-1.5 rounded-full ${isSoldOut ? "bg-white/15 text-white/50 border border-white/20" : "bg-white/20 text-white/90"}`}>
              {isSoldOut ? "Esgotado" : "Disponível"}
            </span>
          </div>

          <div className="mt-4 flex items-end justify-between">
            <p className={`font-outfit font-bold text-[21px] ${isSoldOut ? "text-white/40 line-through" : "text-white"}`}>
            ...
            <button
              ...
              className={`w-10 h-10 rounded-full bg-white flex items-center justify-center shadow-[0_4px_14px_rgba(0,0,0,0.18)] hover:scale-105 active:scale-95 transition-transform ${isSoldOut ? "opacity-40 cursor-not-allowed" : ""}`}
            >
